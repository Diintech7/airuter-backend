...
const generateToken = (user, role = 'user') => {
  const payload = {
    ...
    ...(role === 'partner' && { partnerData: user.partnerData })
  };
  ...
  try {
    ...
    switch (decoded.role) {
      ...
      case 'partner':
        ...
        dashboardRoute = user?.permissions?.includes('partner') ? '/partner/overview' : '/dashboard';
        break;
    ...
    // Check if user is active (for candidates)
    if (decoded.role === 'candidate' && !user.isActive) {
      return res.status(401).json({
        success: false,
