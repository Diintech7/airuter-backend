  ...
  (req, res, next) => {
    passport.authenticate('google', { session: false }, async (err, user, info) => {
      ...
      if (user.role === 'pendingSelection' || !['jobSeeker', 'recruiter'].includes(user.role)) {
        ...
        return res.redirect(`www.airuter.com/auth/callback?token=${token}&requiresRole=true`);
      } else {
        // User already has a valid role, complete auth flow
        return res.redirect(`www.airuter.com/auth/callback?token=${token}&role=${user.role}`);
      }
    })(req, res, next);
  }
);
